...
class ServerThreadTest {
    ...
    @Test
    @DisplayName("Test if acept the exception")
    ...
    @Test
    @DisplayName("Test the exception in the run method")
    void runException()  {
        BlockingQueue producer = new LinkedBlockingQueue();
        assertThrows(IOException.class,() -> new FiltroThread(producer,"server/filro.txt"));
    }
